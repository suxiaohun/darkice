namespace :yys do
  def init_shi_shen
    ...
    # SSR
    shi_shens = []
    shi_shens << { name: '帝释天', kind: 'SSR', sid: '363', cartoon: false }
    shi_shens << { name: '千姬', kind: 'SSR', sid: '356', cartoon: false }
    ...
    # SP
    shi_shens << { name: '蝉冰雪女', kind: 'SP', sid: '362', cartoon: false }
    shi_shens << { name: '夜溟彼岸花', kind: 'SP', sid: '358', cartoon: false }
    shi_shens << { name: '初翎山风', kind: 'SP', sid: '357', cartoon: false }
    shi_shens << { name: '麓铭大岳丸', kind: 'SP', sid: '355', cartoon: false }
    shi_shens << { name: '待宵姑获鸟', kind: 'SP', sid: '354', cartoon: false }
  ...
  def init_patches
    ...
    patches << { version: "v0.3.6", content: "添加帝释天召唤活动", title: "愿成世间·所有之善" }
    ...
    def init_summon_modes
      YysSummonMode.delete_all
      YysSummonMode.create!(code: 'SSR',name: "ssr召唤")
      YysSummonMode.create!(code: 'SP',name: "sp召唤")
      YysSummonMode.create!(code: 'SJ',name: "神眷召唤")

      YysSummon.delete_all
      YysSummon.create!(mode_id: YysSummonMode.first.try(:id),
                        up_count: 3,
                        sid: YysShiShen.ssr.last.try(:sid)
      )
    end
