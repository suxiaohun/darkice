...
class UnicornMetrics::Middleware < Raindrops::Middleware
  ...
  def call(env)
    return metrics_response_text if env['PATH_INFO'] == @metrics_path
  ...
  def metrics_response_text
    body = @registry.as_json.merge(raindrops)

    headers = {}
    headers['Accept'] = 'text/plain'
    headers['Content-Type'] = 'text/plain'
    [ 200, headers, [convert_prometheus_metrics(body)] ]
  end
